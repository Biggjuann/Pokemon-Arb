...
def create_app() -> FastAPI:
    ...
    def deal_detail(request: Request, deal_id: int, db: Session = Depends(get_db)) -> HTMLResponse:
        ...
        if not is_fresh(deal.listing, settings):
            ...
            return templates.TemplateResponse(
                ...
                "stale.html",
                {
                    ...
                    "age": age_label(deal.listing.last_seen_at),
                    "window_hours": display_window(settings).total_seconds() / 3600,
                    "scan_state": _job_state,
                },
                status_code=409,
        ...
        return templates.TemplateResponse(
            ...
            {
                "deal": deal,
                "listing_age": age_label(deal.listing.last_seen_at),
                "settings": get_settings(),
                "risk_weights": RISK_WEIGHTS,
                "statuses": STATUSES,
            },
    ...
    def toggle_keyword(
        keyword_id: int, enabled: bool = Form(...), db: Session = Depends(get_db)
    ) -> RedirectResponse:
        ...
        return [
            {
                ...
                "listing_age": age_label(d.listing.last_seen_at),
            }
            for d in deals
        ]
